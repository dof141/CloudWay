...
const buildExportHTML = (mapDataUrl: string = ''): string => {
  ...
  // 底部二维码 — 项目开源地址
  const repositoryUrl = 'https://github.com/dof141/CloudWay'
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(repositoryUrl)}`
  const footerHTML = `
    <div style="text-align:center;padding:24px 16px 16px;border-top:1px solid #e8e8e8;margin-top:8px;">
      ...
      <div style="font-size:13px;color:#667eea;font-weight:600;margin-bottom:4px;">${t('app.brand')}</div>
      <div style="font-size:11px;color:#aaa;">${repositoryUrl}</div>
